...
export function generatePentestReport(scan: ScanData, findings: FindingData[]) {
  ...
  // Helper for drawing code blocks
  const drawCodeBlock = (title: string, code: string) => {
    ...
    const blockHeight = (splitCode.length * lineHeight) + (padding * 2) + headerHeight;
    ...
    doc.rect(leftMargin, currentY, rightMargin - leftMargin, blockHeight - headerHeight, "F");

    // Code lines
    doc.setFont("courier", "normal");
    doc.setFontSize(8);
    doc.setTextColor(171, 178, 191);
    doc.text(splitCode, leftMargin + 4, currentY + padding + 1.5);

    currentY += (blockHeight - headerHeight) + 6;
  };
...
• EC2 Auditor: Inspects security groups, inbound firewall policies, and network-exposed configurations.`;
  ...
  sortedFindings.forEach((f, idx) => {
    ...
    if (f.description) {
      ...
      doc.setFontSize(9.5);
      doc.setTextColor(74, 85, 104);
      doc.text("Impact Analysis & Description:", leftMargin, currentY);
      currentY += 5;

      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
      doc.setTextColor(colors.darkText[0], colors.darkText[1], colors.darkText[2]);
      const splitDesc = doc.splitTextToSize(f.description, rightMargin - leftMargin);
    ...
    if (remediation) {
      if (remediation.explanation) {
        ...
        doc.text("Remediation Strategy:", leftMargin, currentY);
        currentY += 5;

        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        doc.setTextColor(colors.darkText[0], colors.darkText[1], colors.darkText[2]);
        const splitExpl = doc.splitTextToSize(remediation.explanation, rightMargin - leftMargin);
        doc.text(splitExpl, leftMargin, currentY);
  ...
  doc.setFontSize(14);
  doc.setTextColor(colors.primary[0], colors.primary[1], colors.primary[2]);
  doc.text("5. Conclusion & Recommendations", leftMargin, currentY);
  currentY += 8;
